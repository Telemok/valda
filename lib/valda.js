...
/*
https://github.com/jonschlinkert/is-number (used in React)
...
valda.uint8Array.toHex = Valda.factory((value, argument0) => {
		let result = valda.object.analyse(value, argument0);
		if (!result.name) {
			if (result.value.constructor !== Uint8Array) {
				result.cause = `Value not Uint8Array.`;
				...
				result.value = valda.uint8Array.toHex.convert(result.value);
			}
		}
		return result;
});
valda.uint8Array.toHex.convert = (uint8Array) => {
	let hex = ""
	for(let i = 0; i < uint8Array.length; i++)
		hex += ('0' + uint8Array[i].toString(16)).slice(-2);
	return hex;
}
...
valda.stringMinMax = Valda.factory((value, minimalLengthSymbols = 0, maximalLengthSymbols = Number.MAX_SAFE_INTEGER, argument0) => {
	let result = valda.string.analyse(value, argument0);
	if (!result.name) {
		if (!(result.value.length >= minimalLengthSymbols)) {
			result.cause = `String length (${result.value.length}) must be >= (${minimalLengthSymbols})`;
			result.name = "STRING_LENGTH_IS_LESS_MINIMUM";
		}
		else if (!(result.value <= maximalLengthSymbols)) {
			result.cause = `String length (${result.value.length}) must be <= (${maximalLengthSymbols})`;
			result.name = "STRING_LENGTH_IS_GREATER_MAXIMNUM";
		}
	}
	return result;
});

valda.stringMin = Valda.factory((value, minimalLengthSymbols = 0, argument0) => {
	return valda.stringMinMax.analyse(value, minimalLengthSymbols, Number.MAX_SAFE_INTEGER, argument0);
});

valda.stringMax = Valda.factory((value, maximalLengthSymbols = Number.MAX_SAFE_INTEGER, argument0) => {
	return valda.stringMinMax.analyse(value, 0, maximalLengthSymbols, argument0);
});
...
valda.varName = Valda.factory((value, argument0) => {
		let result = valda.string.analyse(value, argument0);
		if (!result.name) {
			if (!valdaAssertVarNameRegex.test(result.value)) {
...
const valdaAssertHexRegex = /^([0-9A-F]*)$/i;
valda.hex = Valda.factory((value, argument0) => {
		...
		if (!result.name) {
			...
			if (!valdaAssertHexRegex.test(result.value)) {
